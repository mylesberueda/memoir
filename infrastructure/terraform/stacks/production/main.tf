...
module "memorystore" {
  ...
  memory_size_gb = 2 # 2GB for production
...
module "zitadel_gke" {
  ...
  external_domain = "auth.${var.domain}"
...
module "stripe" {
  ...
  stripe_webhook_url            = "https://${var.domain}"
  app_login_url                 = "https://${var.domain}"
}
...
resource "google_secret_manager_secret_version" "terraform_outputs" {
  secret = module.secrets.secret_ids["production/terraform-outputs"]
