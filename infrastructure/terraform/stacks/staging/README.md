...
CI updates values files on push to `dev`. ArgoCD picks up changes via polling
(default 3 min).
...
# 3. Create tunnel
cloudflared tunnel create argocd-staging

# 4. Route DNS (replace with your domain)
cloudflared tunnel route dns argocd-staging argocd-tunnel-staging.yourdomain.com

# 5. Get tunnel token
cloudflared tunnel token argocd-staging
...
2. Domain: `argocd-tunnel-staging.yourdomain.com`
...
| Variable | Value |
|----------|-------|
| `ARGOCD_TUNNEL_STAGING` | `argocd-tunnel-staging.yourdomain.com` |
