...
async fn start(host: &Option<String>, port: &Option<String>) -> crate::Result<()> {
    ...
    // Reflection lets operators run `grpcurl localhost:<port> list` to
    // enumerate every registered service without hand-rolling the proto
    // paths. The encoded FileDescriptorSet is generated by `buf build`
    // (see `gen:protos` in project.json) and shipped through memoir-sdk-rs.
    let reflection_service = ReflectionBuilder::configure()
        .register_encoded_file_descriptor_set(memoir_sdk::FILE_DESCRIPTOR_SET)
        .build_v1()
        .wrap_err("failed to build gRPC reflection service")?;
