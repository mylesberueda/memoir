...
///
/// `None` means no `MEMOIR_BENCH_PROVIDER` was set — the caller skips (a plain
...
/// # Panics
///
/// Panics when `MEMOIR_BENCH_PROVIDER` is set to a paid provider without its API
/// key, or to an unrecognized value — a misconfigured run should fail loudly,
/// not silently skip.
pub fn provider_from_env() -> Option<LlmConfig> {
    let kind = env::var("MEMOIR_BENCH_PROVIDER").ok()?;
    let model_override = env::var("MEMOIR_BENCH_MODEL").ok();
    match kind.to_lowercase().as_str() {
        "ollama" => {
            let url = env::var("MEMOIR_BENCH_OLLAMA_URL").unwrap_or_else(|_| DEFAULT_OLLAMA_URL.to_string());
            let model = model_override.unwrap_or_else(|| DEFAULT_OLLAMA_MODEL.to_string());
            Some(LlmConfig::ollama(url, model))
        }
        "openai" => {
            let api_key = env::var("OPENAI_API_KEY").expect("MEMOIR_BENCH_PROVIDER=openai requires OPENAI_API_KEY");
            let model = model_override.unwrap_or_else(|| DEFAULT_OPENAI_MODEL.to_string());
            Some(LlmConfig::openai(api_key, model))
        }
        "anthropic" => {
            let api_key =
                env::var("ANTHROPIC_API_KEY").expect("MEMOIR_BENCH_PROVIDER=anthropic requires ANTHROPIC_API_KEY");
            let model = model_override.unwrap_or_else(|| DEFAULT_ANTHROPIC_MODEL.to_string());
            Some(LlmConfig::anthropic(api_key, model))
        }
        other => panic!("unknown MEMOIR_BENCH_PROVIDER={other:?} (expected ollama|openai|anthropic)"),
    }
}
